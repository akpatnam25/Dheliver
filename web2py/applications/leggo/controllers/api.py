...
def toggle_visibility():
    q = db.track.id = request.vars.track_id
    db(q).select().first().update_record(is_public = not request.vars.track_id.is_public)

def save():
    q = db.track.id = request.vars.id
    db(q).select().first().update_record(artist= request.vars.artist)
    db(q).select().first().update_record(album =request.vars.album)
